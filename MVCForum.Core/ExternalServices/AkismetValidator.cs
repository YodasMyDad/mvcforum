namespace MvcForum.Core.ExternalServices
{
    using System;
    ...
    public class Validator
    {
        ...
        public bool VerifyKey(string domain)
        {
            ...
            var pars = PreparePars(m_key, domain);
            if (null != pars)
            {
        ...
        protected virtual NameValueCollection PreparePars(Comment comment)
        {
            ...
            result["referrer"] = string.IsNullOrWhiteSpace(comment.referrer)
                ? string.Empty
                : HttpUtility.UrlEncode(comment.referrer);
            result["permalink"] = string.IsNullOrWhiteSpace(comment.permalink)
                ? string.Empty
                : HttpUtility.UrlEncode(comment.permalink);
            result["comment_type"] = string.IsNullOrWhiteSpace(comment.comment_type)
                ? string.Empty
                : HttpUtility.UrlEncode(comment.comment_type);
            result["comment_author"] = string.IsNullOrWhiteSpace(comment.comment_author)
                ? string.Empty
                : HttpUtility.UrlEncode(comment.comment_author);
            result["comment_author_email"] = string.IsNullOrWhiteSpace(comment.comment_author_email)
                ? string.Empty
                : HttpUtility.UrlEncode(comment.comment_author_email);
            result["comment_author_url"] = string.IsNullOrWhiteSpace(comment.comment_author_url)
                ? string.Empty
                : HttpUtility.UrlEncode(comment.comment_author_url);
            result["comment_content"] = string.IsNullOrWhiteSpace(comment.comment_content)
                ? string.Empty
                : HttpUtility.UrlEncode(comment.comment_content);
